...
<!-- 
    <?php/*
      ...
      if (!$conn) {die("Connection failed: " . mysqli_connect_error());}// Check connection       /*  echo "<script>alert('Sikeresen bejelentkeztél!')</script>"; */
 ...
 -->
  ...
  <?php
    if(isset($_POST['submit'])){
      ...
      $db = mysqli_select_db($con,"reftarka_database");
      $query = mysqli_query($con,"select * from login"); 
      while($temp = mysqli_fetch_array($query)){
        if($uname == $temp[1] && $pass == $temp[2]){
          $flag = 1;
          break;
        }else{ $flag = 0; }
      }
      if($flag==1){header('location:home.html');}
 ...
 -->
  ...
  <?php
    if(isset($_POST['submit'])){
      ...
      $con = mysqli_connect("localhost","root","");
      $db = mysqli_select_db($con,"reftarka_database");
      $query = mysqli_query($con,"select * from loginadmin"); 
      while($temp = mysqli_fetch_array($query)){
        if($uname == $temp[1] && $pass == $temp[2]){
          $flag = 1;
          break;
...
>>>>>>> 6b9de58d49fec0257ef4875f79fbe6670b3be209:thePhP-file_forLoginForm.php
